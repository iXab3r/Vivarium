...
| teamcity:pipeline-shape | shared Build Number -> four Compile configurations -> Release -> Publish | TeamCity | TeamCity Expert, Test Steward | seven simplified configurations imported; Windows increment evidence green | one counter and one source revision stamp the same version into every RID; Release contains no compile/test step; Publish contains no packaging step |
