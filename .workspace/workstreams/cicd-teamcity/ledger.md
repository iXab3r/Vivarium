...
| release-contract:docker | server container distribution and update | release | TeamCity, Platform, Security | required destination parameters missing; first registry publish pending | exact linux-x64 release bytes, non-root runtime, persistent data volume, versioned + latest tags |
...
| teamcity:pipeline-shape | Compile -> Release -> independent GitHub and Docker publishers | TeamCity | TeamCity Expert, Test Steward | independent graph imported; Docker destination parameters pending | one Compile counter and source revision stamp every RID; Release contains no compile/test step; either publisher can run alone |
