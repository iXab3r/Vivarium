...
internal sealed class GitHubReleasePublisher
{
    ...
    private LocalAsset[] ExpectedAssets()
    {
        ...
        return ReleaseLayout.ReleaseAssetNames()
            .Select(name =>
            {
                ...
                return new LocalAsset(name, path, content.Length, digest);
            })
            .ToArray();
    }
        ...
        IReadOnlyCollection<LocalAsset> expected)
    {
        ...
        {
            if (!expectedByName.TryGetValue(remote.Name, out var local) ||
                remote.Size != local.Size ||
                !string.Equals(remote.Digest, local.Digest, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
                    $"Release contains an unexpected or byte-mismatched asset '{remote.Name}'.");
            }
        }
    }
