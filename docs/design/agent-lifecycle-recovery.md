...
| `ALR-002` | One Stop command is treated as both graceful cancellation and hard termination. Cleanup is either skipped or allowed to hang forever. | Separate graceful and force modes, persist their per-phase deadlines, preserve the first reason, and expose honest intermediate states. | **Implemented for the current process contract.** Graceful sends `SIGTERM`/`CloseMainWindow`, runs only `ALWAYS` cleanup, and never grants hard-kill authority; explicit force skips cleanup and kills the process tree. Native containment evidence remains under `ALR-001`. |
